...
class DesktopPlatformServices(private val settings: () -> AppSettings) : PlatformServices {
    ...
    override val dataDirectory: String = when (deviceType) {
        DeviceType.WINDOWS -> File(System.getenv("APPDATA") ?: System.getProperty("user.home"), "DropBoxx").absolutePath
        DeviceType.MACOS -> File(System.getProperty("user.home"), "Library/Application Support/DropBoxx").absolutePath
        else -> File(System.getProperty("user.home"), ".local/share/dropboxx").absolutePath
    }.also { File(it).mkdirs() }

    override val defaultSaveDirectory: String = File(System.getProperty("user.home"), "Downloads/DropBoxx").absolutePath
    ...
    override fun setLaunchAtStartup(enabled: Boolean): Boolean {
        ...
        val startup = File(System.getenv("APPDATA"), "Microsoft/Windows/Start Menu/Programs/Startup/DropBoxx.lnk")
        return runCatching {
