...
fun SettingsScreen(viewModel: SettingsViewModel = koinViewModel()) {
    ...
    LazyColumn(Modifier.fillMaxSize(), contentPadding = PaddingValues(16.dp), verticalArrangement = Arrangement.spacedBy(4.dp)) {
        ...
        if (viewModel.supportsTray) {
            item { SwitchRow("Keep running in the tray", "Closing the window keeps DropNest receiving in the background", s.minimizeToTray, viewModel::setMinimizeToTray) }
            item { SwitchRow("Start with Windows", "Launch DropNest when you sign in", s.launchAtStartup, viewModel::setLaunchAtStartup) }
        }
