...
class AndroidPlatformServices(private val context: Context, private val launcherActivity: Class<*>) : PlatformServices {
    ...
    override val dataDirectory: String = File(context.filesDir, "dropboxx").apply { mkdirs() }.absolutePath
    override val defaultSaveDirectory: String = "Download/DropBoxx"
    ...
    init {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            notifications.createNotificationChannel(NotificationChannel(CHANNEL_SERVICE, "Background service", NotificationManager.IMPORTANCE_LOW).apply { description = "Shown while DropBoxx is ready to receive" })
        }
    }
    ...
    override fun setMulticastEnabled(enabled: Boolean) {
        if (enabled) {
            if (multicastLock == null) multicastLock = wifi.createMulticastLock("dropboxx").apply { setReferenceCounted(false); acquire() }
        } else {
    ...
    override fun setKeepAwake(enabled: Boolean) {
        if (enabled) {
            if (wakeLock == null) wakeLock = power.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "dropboxx:transfer").apply { acquire(4 * 60 * 60 * 1000L) }
            if (wifiLock == null) {
                ...
                wifiLock = wifi.createWifiLock(mode, "dropboxx:transfer").apply { acquire() }
            }
        } else {
